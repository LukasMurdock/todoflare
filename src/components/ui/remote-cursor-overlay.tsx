...
import { useEditorContainerRef, useEditorVersion, usePluginOption } from 'platejs/react';
...
export function RemoteCursorOverlay() {
  ...
  return (
    <RemoteCursorErrorBoundary key={editorVersion}>
      <RemoteCursorOverlayContent />
    </RemoteCursorErrorBoundary>
  );
}

class RemoteCursorErrorBoundary extends React.Component<
...
> {
  ...
  componentDidCatch(error: unknown) {
    if (
      ...
      error.message.includes('Unable to find the path for Slate node')
    ) {
      return;
    }
